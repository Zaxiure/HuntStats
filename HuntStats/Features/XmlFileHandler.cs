...
{
    [XmlElement(ElementName = "Attr")] public List<Attr> Atrributes { get; set; }
...
public class XmlFileQueryHandler : IRequestHandler<XmlFileQuery, GeneralStatus>
{
    ...
    public async Task<GeneralStatus> Handle(XmlFileQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!sameFile)
            {
                ...
                var reader = new XmlSerializer(typeof(Attributes));
                var fileText = File.ReadAllText(huntFileTempPath);
                var file = new StringReader(fileText);


                var overview = (Attributes)reader.Deserialize(file);
                ...
                foreach (var attr in overview.Atrributes) attributes.Add(attr.Name, attr.Value);
                ...
                for (var i = 0; i < numberOfAccolades; i++)
    ...
    public bool CheckSameMatch(string filePath1, string filePath2)
    {
        ...
        foreach (var attr in overview2.Atrributes) attributes2.Add(attr.Name, attr.Value);
        ...
        for (var i = 0; i < numberOfAccolades; i++)
            ...
            {
                total += 1;
                if (attributes2.ContainsKey(accoladeEntry.Key) && attributes2[accoladeEntry.Key] == accoladeEntry.Value) succes += 1;
        ...
        for (var i = 0; i < numberOfEntries; i++)
            ...
            {
                total += 1;
                if (attributes2.ContainsKey(missionBagEntry.Key) && attributes2[missionBagEntry.Key] == missionBagEntry.Value) succes += 1;
            }
        ...
        for (var i = 0; i < numberOfTeams; i++)
            ...
            {
                total += 1;
                if (attributes2.ContainsKey(playerEntry.Key) && attributes2[playerEntry.Key] == playerEntry.Value) succes += 1;
            }

        var percentage = Convert.ToInt32((double)succes / total * 100);
        if (percentage > 88) return true;
